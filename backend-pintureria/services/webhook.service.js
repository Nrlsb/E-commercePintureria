...
export const processPaymentNotification = async (topic, eventId) => {
  ...
  try {
    // --- MEJORA: Verificar si el evento ya fue procesado o falló ---
    ...
    if (paymentInfo.status === 'approved') {
      ...
      const orderDataResult = await dbClient.query(`
        SELECT o.*, u.email FROM orders o JOIN users u ON o.user_id = u.id WHERE o.id = $1`, [orderId]);
      
      if (orderDataResult.rows.length > 0) {
        const order = orderDataResult.rows[0];
        const itemsForEmailResult = await dbClient.query(`
          SELECT p.name, oi.quantity, oi.price FROM order_items oi JOIN products p ON p.id = oi.product_id WHERE oi.order_id = $1`, [orderId]);
        order.items = itemsForEmailResult.rows;
        await sendOrderConfirmationEmail(order.email, order);
      }
      
      await dbClient.query('COMMIT');
      // --- MEJORA: Marcar como procesado ---
      ...
      // --- MEJORA: Marcar como procesado aunque no esté aprobado para no reintentar ---
      await db.query("UPDATE webhook_events SET status = 'processed', error_message = NULL WHERE event_id = $1", [eventId]);
    }
  } catch (error) {
    if (dbClient) await dbClient.query('ROLLBACK');
    logger.error(`Error procesando webhook para el pago ${eventId}:`, error);
    // --- MEJORA: Guardar el error en la base de datos ---
