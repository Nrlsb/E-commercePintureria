...
const ProductFilters = ({ category }) => {
  const { allProducts, filters, setFilters, setSortOption, fetchProducts } = useProductStore();
  const [localFilters, setLocalFilters] = useState(filters);
  const [brands, setBrands] = useState([]);

  // Derivamos las marcas disponibles de la lista de productos
  useEffect(() => {
  ...
  const handleSortChange = (e) => {
    ...
    fetchProducts(category); // Re-fetch al cambiar el orden
  };
  
  const applyFilters = () => {
    setFilters(localFilters);
    fetchProducts(category); // Re-fetch al aplicar filtros
  };
